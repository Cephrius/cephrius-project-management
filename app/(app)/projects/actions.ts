...
export async function createBuilder(nameRaw: string): Promise<{ ok: boolean; data?: IdName; message?: string }> {
    ...
    return { ok: false, message: ins.error?.message ?? "Failed to save builder." };
}

export async function createSubdivision(nameRaw: string): Promise<{ ok: boolean; data?: IdName; message?: string }> {
    const supabase = createClient();
    const { data: auth } = await supabase.auth.getUser();
    if (!auth.user) redirect("/login");
    ...
    return { ok: false, message: ins.error?.message ?? "Failed to save subdivision." };
}
...
export async function createProject(formData: FormData) {
    ...
    const subdivision_id = String(formData.get("subdivision_id") || "").trim() || null;
    const subdivision_name_raw = String(formData.get("subdivision_name") || "").trim();
    ...
    if (finalBuilderId) {
        ...
        if (!created.ok || !created.data) return { ok: false, message: created.message ?? "Failed to save builder." };
    ...
    if (finalSubdivisionId) {
        ...
        if (!created.ok || !created.data) {
            return { ok: false, message: created.message ?? "Failed to save subdivision." };
        }
