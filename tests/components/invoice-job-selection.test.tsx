...
vi.mock("@/app/(jobsyte-app)/invoices/actions", () => ({
  createInvoiceForBuilder: mocks.createInvoice,
}));
  ...
  beforeEach(() => {
    ...
    mocks.from.mockImplementation((table: string) => {
      const data =
        table === "projects"
          ? [
              { id: "p1", project_address: "10 Oak Drive", subdivision: "Oak" },
              {
                id: "p2",
                project_address: "20 Pine Drive",
                subdivision: "Pine",
              },
            ]
          : table === "jobs"
  ...
  it("starts unchecked, totals explicit choices across projects, and submits only those choices", async () => {
    render(
      <CreateInvoiceByBuilder
        builders={[{ id: "b1", name: "Oak Builders" }]}
        initialContractor={{
    ...
    fireEvent.change(screen.getByRole("combobox", { name: "Builder" }), {
      target: { value: "b1" },
    });
    ...
    screen
      .getAllByRole("button", { name: "Create Invoice" })
    ...
    fireEvent.change(screen.getByLabelText("Billing Address"), {
      target: { value: "Builder office" },
    });
    fireEvent.click(
      screen.getAllByRole("button", { name: "Create Invoice" })[0],
    );
    await waitFor(() => expect(mocks.createInvoice).toHaveBeenCalledOnce());
    expect(mocks.replace).toHaveBeenCalledWith("/invoices/invoice-one");
    expect(
      (mocks.createInvoice.mock.calls[0][0] as FormData).getAll("job_ids"),
    ).toEqual(["j2"]);
  });
});
